...
def check_user(message):
    ...
    if user_id in sudo_users:
        ...
        text = "<b>Not a Authorized User</b>\nMade With Love By Doraemon From 22nd Century"
...
def multi_rec(app, message):
    ...
    video_opts_2 = '-to'
    video_opts_3 = '-map 0:v:0 -map 0:a -metadata:s:a title=[KMAE].(AAC.2.0~192Kbps)]'
    audio = "-".join(iptv_data[channel][0]["audio"])
    filename = f'{iptv_data[channel][0]["title"]} - {title} - {ind_time()} [{iptv_data[channel][0]["quality"]}] [x264] {iptv_data[channel][0]["ripType"]} [{audio}][{GROUP_TAG}].mkv'
